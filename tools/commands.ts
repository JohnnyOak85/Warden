...
export const executeCommand = (message: Message) => {
  try {
    ...
    if (!command) {
      message.channel.send(getResponse('noCommand', 'That command does not exist.'));
      return;
    }

    command.execute(message, args);
  } catch (error) {
    message.channel.send(getResponse('commandError', 'There was an error trying to execute that command!'));
    logError(error);
  }
};
